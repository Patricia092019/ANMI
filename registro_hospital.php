...
if (!empty($_POST)) {
    ...
    if (empty($_POST['Hospital']) || empty($_POST['mqprogramados']) || empty($_POST['Mqabastecido'])) {
        ...
        if (is_numeric($dni) and $dni != 0) {
            $query = mysqli_query($conexion, "SELECT * FROM hospitales where Hospital = '$hospital'");
            $result = mysqli_fetch_array($query);
        }
        if ($result > 0) {
            $alert = '<div class="alert alert-danger" role="alert">
                                    El Hospital ya existe
                                </div>';
        } else {
            $query_insert = mysqli_query($conexion, "INSERT INTO hospitales (Hospital,mqprogramados,Mqabastecido,abastecimiento) values ('$hospital', '$mqprogramados', '$Mqabastecido', '$abastecimiento')");
            if ($query_insert) {
...
<div class="container-fluid">
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h3 class="text-black">Registro de Nuevo Hospital </h3>
                ...
                <div class="card-body">
                    <form action="" method="post" autocomplete="off">
                        ...
                        <div class="form-group">
                            <label for="nombre">Nombre de Hospital </label>
                            <input type="text" placeholder="Ingrese Nombre" name="Hospital" id="Hospital" class="form-control">
                        </div>
                        <div class="form-group">
                        ...
                        <input type="submit" value="Guardar Hospital" class="btn btn-primary">
                    </form>
                </div>
            </div>
        </div>
