...
if (empty($_REQUEST['id'])) {
    ...
    $query_producto = mysqli_query($conexion, "SELECT nombre , codigo,unidades FROM producto WHERE codigo = $id_producto");
...
if (!empty($_POST)) {
    $alert = "";
    if (!empty($_POST['codigo']) || !empty($_POST['nombre']) || !empty($_POST['unidades'])) {
        $nombre = $_POST['nombre'];
        $codigo = $_POST['codigo'];
        $unidades = $_POST['unidades'];
        $producto_id = $_GET['id'];
        $usuario_id = $_SESSION['idUser'];
        $query_insert = mysqli_query($conexion, "INSERT INTO producto (nombre, codigo,unidades,usuario_id) VALUES ($nombre, $codigo, $unidades, $usuario_id)");
        if ($query_insert) {
