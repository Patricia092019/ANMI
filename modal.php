...
if (!empty($_POST)) {
  ...
  if ($_POST['action'] == 'infoProducto') {
    ...
    $query = mysqli_query($conexion, "SELECT nombre, codigo, unidades FROM producto WHERE id = $producto_id");
...
// Eliminar Producto
  if ($_POST['action'] == 'delProduct') {
    ...
    $query_delete = mysqli_query($conexion, "UPDATE producto SET estado = 0 WHERE id = $idproducto");
    ...
    while ($data = mysqli_fetch_assoc($query_detalle_temp)) {
      ...
      $total = round($total + $precioTotal, 2);
        ...
        $detalleTabla .='<tr>
            ...
            <td class="textcenter">'.$data['unidades'].'</td>
...
if ($_POST['action'] == 'addcita') {
  $fecha = $_POST['fecha'];
  $hora = $_POST['hora'];
  $licitacion = $_POST['referencia'];
  $laboratorio = $_POST['laboratorio'];
  $bodega = $_POST['bodega'];
  $usuario_id = $_SESSION['idUser'];

  $query_insert = mysqli_query($conexion, "INSERT INTO cita (fecha,hora, laboratorio,referencia, bodega) VALUES ('$fecha','$hora','$licitacion','$laboratorio','$bodega ')");
  if ($query_insert) {
    $codCliente = mysqli_insert_id($conexion);
    $msg = $codCliente;
  }else {
